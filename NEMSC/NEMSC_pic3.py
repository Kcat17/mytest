...
plt.plot(t, yNEMSC[:, 0], lineclass[0], color=color[0], label="正常(N)" )
plt.plot(t, yNEMSC[:, 1], lineclass[1], color=color[1], label="好奇(E)")
plt.plot(t, yNEMSC[:, 2], lineclass[2], color=color[2], label="轻度恐慌(M)" )
plt.plot(t, yNEMSC[:, 3], lineclass[3], color=color[3], label="重度恐慌(S)" )
plt.plot(t, yNEMSC[:, 4], lineclass[4], color=color[4], label="冷静(C)" )
# for i in range(5):
# 	beta-=0.05
# 	yNEMSC = odeint(dyNEMSC, Y1, t, args=(alpha, theta, beta, delta, phi, gamma, epsilon2,epsilon3,epsilon4,epsilon5, omega, iota,kappa))
# 	plt.plot(t, yNEMSC[:, 3], lineclass[i], color=color[i], label="beta=" +str('{:.2f}'.format(beta)))
...
plt.ylabel('各类人群比例(%)',fontsize=14)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(loc='upper right',fontsize=14)

# plt.title("S_Beta", fontsize=24)
# plt.savefig("D:/shot/shotimage      "+saveP+".png",dpi=300)
plt.show()
