...
for i in range(5):
	alpha-=0.1
	yNEMSC = odeint(dyNEMSC, Y1, t, args=(alpha, theta, beta, delta, phi, gamma, epsilon2,epsilon3,epsilon4,epsilon5, omega, iota,kappa))
	plt.plot(t, yNEMSC[:, 3], lineclass[i], color=color[i], label="alpha=" +str('{:.2f}'.format(alpha)))
...
# plt.title("S_epsilon", fontsize=24)
plt.xlabel('迭代次数m',fontsize=14)
plt.ylabel('重度恐慌人群(S)比例(%)',fontsize=14)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(loc='lower right',fontsize=14)
plt.savefig("D:/PycharmPythonProjects/curve/venv/NEMSC/results/s_alpha      "+saveP+".png",dpi=300)
plt.show()
